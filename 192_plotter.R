...
#scripts to plot lovely mutation spectra in 96 or 192 channel format, using base R. Header is important to ensure everything is ordered correctly.
#run as
...
plotSpectrum96<-function(x,z,yLowerBound=0,yUpperBound=0,ti="",labs=0,bc,labScale=1.6,xlab="",ylab="",RefHeight=-1,AltHeight=-2){
  ...
  for (sb in 1:6) {
    ...
    if (sb == 2 || sb == 5 || sb == 8 || sb == 11) {
     ...
     mtext(botText[text_counter],side=1,at=pos_counter,col="white",cex=labScale,line=RefHeight,font=2)
